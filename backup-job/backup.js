...
async function buildExcel(data) {
  ...
  // ── ID → naam lookup tables (Vouchers/Voucher Lines ko readable banane ke liye) ──
  const partyName = {};
  ...
  const readableVouchers = (data.vouchers || []).map(function (v) {
    return {
  ...
  const readableVoucherLines = (data.voucher_lines || []).map(function (l) {
    return {
